...
- ターム区分とカウント可否は `checked_in_at`（来場時刻）から**都度導出**し、`sessions` には保存しない。判定ロジックは `packages/shared/src/venue-schedule.ts`（`classifyTerm` / `countsTowardParticipation`）に集約し、API・フロント双方が同じ実装を使う。
...
| 参加回数         | タームごとの参加を数えた累計（朝＋昼で2）。物理スキルカードのチェック数に対応 |
| 来場日数         | 重複排除した開催日数。参加回数とは別指標                                    |

...
└──────────┬──────────────────┘
           ...
           ▼
┌─────────────────────────────┐
│ 内製DB (Neon PostgreSQL)    │  ← アクティベート以降の主データソース
└─────────────────────────────┘
```
...
| DB接続               | Cloudflare Hyperdrive（接続プーリング・低レイテンシ）      |
| ORM                  | Drizzle ORM                                                |
...
- **Neon**: サーバーレスPostgres、ブランチング機能（DB version control）
- **Drizzle ORM**: TypeScriptネイティブ、エッジ環境対応、Honoコミュニティの主流
...
| R3  | Google Sheets API書き戻し失敗時のデータ不整合                                    | 中     | 中       | MVPでは書き戻し失敗時にユーザーへエラー表示＆再試行。本格的にはリトライキュー導入を検討                                    |
...
| 週     | マイルストーン                                                                                              |
| ------ | ----------------------------------------------------------------------------------------------------------- |
| W1     | モノレポ・CI/CD・Cloudflare/Vercel/Neon環境構築。Hono → Hyperdrive → Neon E2E疎通。Google Sheets API疎通PoC |

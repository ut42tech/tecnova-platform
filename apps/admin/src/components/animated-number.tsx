...
// 値は整数想定（来場者数・参加回数）。桁揃えは呼び出し側で tabular-nums を付ける。
// 注: サマリカードは再フェッチ（更新ボタン・日付/期間変更）のたびに loading→ok で再マウントされるため、
// その都度 0 から数え直す。これは「データが更新された」フィードバックとして許容する意図的な挙動
// （止めたい場合は再フェッチ中も前回データを残してカードをアンマウントしない設計が必要）。
